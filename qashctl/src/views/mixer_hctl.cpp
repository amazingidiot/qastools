	...
	if ( _view_setup != 0 ) {
		...
		if ( _view_setup->wdg_style_db != 0 ) {
			// Get playback / capture colors
			{
				...
				_tree_model->set_snd_dir_foreground ( 0,
			...
			}
			{
				...
				const QPalette & pal (
					_view_setup->wdg_style_db->palette ( style_id ) );
				_tree_model->set_snd_dir_foreground ( 1,
					pal.color ( QPalette::WindowText ) );
			}
		}
		setup_view();
...
void
Mixer_HCTL::tree_element_selected (
...
void
Mixer_HCTL::table_element_selected (
	const QModelIndex & idx_n )
{
