	...
	::QSnd::Mixer_HCTL_Elem * elem_n )
{
	...
	if ( _snd_elem != 0 ) {
		connect ( _snd_elem, SIGNAL ( sig_values_changed() ),
			this, SLOT ( update_value_from_source() ) );
	}
}


void
Mixer_HCTL_Proxy::set_elem_idx (
	unsigned int idx_n )
{
	_elem_idx = idx_n;
}
