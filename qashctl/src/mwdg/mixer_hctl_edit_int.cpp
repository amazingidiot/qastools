...
void
Mixer_HCTL_Edit_Int::set_inputs_setup (
...
Mixer_HCTL_Edit_Int::create_proxies_group (
	...
	bool multi_n )
{
	...
	for ( unsigned int ii=0; ii < num_channels; ++ii ) {
		Mixer_HCTL_Int_Proxy_Column * pcol (
			new Mixer_HCTL_Int_Proxy_Column );
				...
				if ( elem_n->dB_range ( &db_min, &db_max ) == 0 ) {
					...
					if ( ( db_min < 0 ) && ( db_max > 0 ) ) {
						...
						pstyle->slider_minimum_idx =
							pslider->integer_to_index (
								elem_n->ask_int_from_dB ( 0 ) );
					} else {
						if ( db_max <= 0 ) {
							pstyle->slider_has_minimum = true;
		...
		if ( ( ii == 0 ) && ( num_channels > 1 ) ) {
			::Wdg::Pad_Proxy_Switch * pswitch (
				new ::Wdg::Pad_Proxy_Switch );
			...
			connect ( pswitch, SIGNAL ( sig_switch_state_changed ( bool ) ),
				sppg, SLOT ( set_joined ( bool ) ) );
		}
...
Mixer_HCTL_Edit_Int::setup_single ( )
{
	...
	_proxies_groups.append (
		create_proxies_group ( elem, false ) );
...
Mixer_HCTL_Edit_Int::setup_multi ( )
{
	...
	for ( unsigned int eii=0; eii < num_elems; ++eii ) {
		...
		_proxies_groups.append (
			create_proxies_group ( elem, true ) );
	}
...
QString
Mixer_HCTL_Edit_Int::integer_string (
	long value_n ) const
{
		...
		if ( _sliders_pad->focus_info().has_focus ) {
			...
			if ( idx < _proxies_groups.size() ) {
				_focus_proxies_group =
					static_cast < Mixer_HCTL_Int_Proxies_Group * > ( _proxies_groups[idx] );
...
bool
Mixer_HCTL_Edit_Int::context_menu_start (
	const QPoint & pos_n )
{
	bool res ( false );
...
void
Mixer_HCTL_Edit_Int::footer_label_selected (
...
bool
Mixer_HCTL_Edit_Int::eventFilter (
	QObject * watched_n,
	QEvent * event_n )
{
	bool filtered ( false );
