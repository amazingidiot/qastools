...
int
Mixer_HCTL_Table_Model::rowCount (
...
int
Mixer_HCTL_Table_Model::columnCount (
	const QModelIndex & parent_n ) const
{
	int res ( 0 );
	if ( !parent_n.isValid() ) {
	...
	int role_n ) const
{
	...
	if ( !index_n.isValid() ||
		index_n.parent().isValid() ||
		( row < 0 ) || ( row >= _elems.size() ) ||
		( col < 0 ) || ( col >= (int)_num_columns ) )
	{
		return res;
	}
...
QVariant
Mixer_HCTL_Table_Model::headerData (
	int section_n,
	Qt::Orientation orientation_n,
	int role_n ) const
{
	QVariant res;
	if ( ( orientation_n == Qt::Vertical ) ||
		( section_n < 0 ) ||
...
void
Mixer_HCTL_Table_Model::set_snd_mixer (
...
void
Mixer_HCTL_Table_Model::set_iface_type_idx (
	unsigned int type_n )
{
