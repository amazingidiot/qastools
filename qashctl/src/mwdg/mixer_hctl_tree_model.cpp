...
void
Mixer_HCTL_Tree_Model::set_snd_dir_foreground (
	unsigned int dir_n,
...
unsigned int
Mixer_HCTL_Tree_Model::index_data (
...
QModelIndex
Mixer_HCTL_Tree_Model::elem_index (
	::QSnd::Mixer_HCTL_Elem * elem_n ) const
{
	QModelIndex res;
...
QModelIndex
Mixer_HCTL_Tree_Model::elem_desc_index (
	const QString & iface_name_n,
...
void
Mixer_HCTL_Tree_Model::set_snd_mixer (
	::QSnd::Mixer_HCTL * snd_mixer_n )
{
...
Mixer_HCTL_Tree_Model::load ( )
{
	...
	for ( unsigned int ii=0; ii < num_elems; ++ii ) {
		...
		if ( tgrp != 0 ) {
			::QSnd::Mixer_HCTL_Elem_Group * grp (
				new ::QSnd::Mixer_HCTL_Elem_Group );
	...
	for ( unsigned int tii=0; tii < num_types; ++tii ) {
		...
		type_item->setText (
			snd_mixer()->info_db()->iface_display_name (
		...
		if ( !match || ( gii == ( lst_n.size() -  1 ) ) ) {
			if ( span > 0 ) {
				for ( unsigned int jj=0; jj < span; ++jj ) {
					::QSnd::Mixer_HCTL_Elem_Group * grp_cur ( lst_n[span_start + 1 + jj] );
...
QBrush
Mixer_HCTL_Tree_Model::get_foreground (
