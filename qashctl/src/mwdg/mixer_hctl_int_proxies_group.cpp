...
Mixer_HCTL_Int_Proxies_Group::mcolumn (
	unsigned int idx_n ) const
{
	return static_cast < ::MWdg::Mixer_HCTL_Int_Proxy_Column * > ( column ( idx_n ) );
}


void
Mixer_HCTL_Int_Proxies_Group::set_joined (
	bool flag_n )
{
	if (  num_columns() < 2 ) {
		return;
...
void
Mixer_HCTL_Int_Proxies_Group::level_volumes (
...
void
Mixer_HCTL_Int_Proxies_Group::set_notify_value_change (
	bool flag_n )
{
