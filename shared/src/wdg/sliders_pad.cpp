...
_image_alloc ( isg_alloc_n )
{
...

	{
		...
		connect (
			lheader, SIGNAL ( sig_label_selected ( unsigned int, unsigned int ) ),
			this, SLOT ( header_label_selected ( unsigned int, unsigned int ) ) );

		connect (
			lfooter, SIGNAL ( sig_label_selected ( unsigned int, unsigned int ) ),
	...
	::dpe::Image_Allocator * alloc_n )
{
	...
	for ( unsigned int ii=0; ii < num; ++ii ) {
		...
		{
			::Wdg::DS_Slider * swdg (
				dynamic_cast < ::Wdg::DS_Slider * > ( wdg ) );
			if ( swdg != 0 ) {
		...
		{
			::Wdg::DS_Switch * swdg (
				dynamic_cast < ::Wdg::DS_Switch * > ( wdg ) );
			if ( swdg != 0 ) {
				swdg->set_image_alloc ( image_alloc() );
				continue;
			}
...
void
Sliders_Pad::set_footer_visible (
	bool flag_n )
{
...
Sliders_Pad::create_widgets ( )
{
	...
	for ( unsigned int ii=0; ii < num_groups; ++ii ) {
		...
		for ( unsigned int jj=0; jj < num_cols; ++jj ) {
			...
			if ( sppc->show_value_string() && sppc->has_slider() ) {
				connect (
					sppc, SIGNAL ( sig_value_string_changed ( ) ),
					sp_col, SLOT ( update_footer_label() ) );
			}
	...
	for ( unsigned int ii=0; ii < num_groups; ++ii ) {
		...
		// Setup header labels variables
		{
			...
			if ( wdg_style_db() != 0 ) {
				lbl.col_fg = wdg_style_db()->color (
					sppg->style_id(), QPalette::WindowText );
			} else {
		...
		for ( unsigned int jj=0; jj < num_cols; ++jj ) {
			...
			lbl.tool_tip = sppg->tool_tip();
			if ( wdg_style_db() != 0 ) {
				lbl.col_fg = wdg_style_db()->color ( sppg->style_id(), QPalette::WindowText );
			} else {
				lbl.col_fg = palette().color ( QPalette::WindowText );
			}
			{
				unsigned int llength[2] = { 0, 0 };
				llength[0] = footer_cast()->label_str_length_px_max ( sppc->value_min_string() );
				llength[1] = footer_cast()->label_str_length_px_max ( sppc->value_max_string() );
	...
	for ( unsigned int gii=0; gii < num_groups; ++gii ) {
		...
		for ( unsigned int cii=0; cii < sppg->num_columns(); ++cii ) {
			...
			if ( sppc->has_slider() ) {
				...
				connect ( spps, SIGNAL ( sig_enabled_changed ( bool ) ),
					sl_wdg, SLOT ( setEnabled ( bool ) ) );
			...
			if ( sppc->has_switch() ) {
				...
				::Wdg::DS_Switch * sw_wdg (
					new ::Wdg::DS_Switch ( 0, _image_alloc ) );
...
void
Sliders_Pad::header_label_selected (
	unsigned int group_idx_n,
	unsigned int column_idx_n )
{
...
void
Sliders_Pad::footer_label_selected (
	unsigned int group_idx_n,
	unsigned int column_idx_n )
{
