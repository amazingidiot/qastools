...
void
Mixer_Simple::append_elem_filter (
	::QSnd::Mixer_Simple_Filter * filter_n )
{
	if ( filter_n != 0 ) {
	...
	const QString & dev_name_n )
{
	...
	err = snd_hctl_open (
		&_snd_hctl,
		dev_name_n.toLocal8Bit().constData(),
		SND_CTL_NONBLOCK );
...
void
Mixer_Simple::destroy_mixer_elem (
	Mixer_Simple_Elem * elem_n )
{
...
Mixer_Simple_Elem *
Mixer_Simple::create_mixer_elem (
...
void
Mixer_Simple::set_socked_notifiers_enabled (
	bool flag_n )
{
	for ( int ii=0; ii < _socket_notifiers.size(); ++ii ) {
	...
	if ( event_n->type() == ::QSnd::evt_update_values_request ) {
		...
		if ( !_update_requested ) {
			...
			QCoreApplication::postEvent (
				this, new QEvent ( ::QSnd::evt_update_values ) );
		}
		...
		if ( !_reload_requested ) {
			...
			QCoreApplication::postEvent (
				this, new QEvent ( ::QSnd::evt_reload ) );
		}
...
int
Mixer_Simple::alsa_callback_mixer (
