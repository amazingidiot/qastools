...
QString
Mixer_HCTL_Elem::enum_item_display_name (
	unsigned int enum_idx_n ) const
{
	return QCoreApplication::translate (
		"ALSA::Enum_Value", enum_item_name ( enum_idx_n ) );
}
...
void
Mixer_HCTL_Elem::set_display_name (
...
void
Mixer_HCTL_Elem::set_switch_state (
	unsigned int idx_n,
	bool state_n )
{
...
void
Mixer_HCTL_Elem::set_switch_all (
	bool state_n )
{
...
void
Mixer_HCTL_Elem::invert_switch_state (
	unsigned int idx_n )
{
...
void
Mixer_HCTL_Elem::set_integer (
	unsigned int idx_n,
	long value_n )
{
	//::std::cout << "Mixer_HCTL_Elem::set_integer " << idx_n << " value " << value_n << "\n";
...
void
Mixer_HCTL_Elem::set_integer_all (
	long value_n )
{
	unsigned int num ( count() );
	for ( unsigned int ii=0; ii < num; ++ii ) {
...
Mixer_HCTL_Elem::level_integers ( )
{
	...
	for ( unsigned int ii=0; ii <  num; ++ii ) {
		...
		if ( ( vol_mod_accu >= num_int ) ||
			( vol_mod_accu <= (-num_int) ) )
		{
...
long
Mixer_HCTL_Elem::ask_dB_from_int (
	long int_value_n ) const
{
	...
	snd_ctl_convert_to_dB (
		snd_hctl_ctl ( _snd_hctl ),	elem_id,
		int_value_n, &res );
...
long
Mixer_HCTL_Elem::ask_int_from_dB (
	long db_value_n,
	int round_dir_n ) const
{
	long res ( 0 );
...
void
Mixer_HCTL_Elem::set_enum_index (
...
void
Mixer_HCTL_Elem::set_enum_index_all (
	unsigned int enum_idx_n )
{
	unsigned int num ( count() );
	for ( unsigned int ii=0; ii < num; ++ii ) {
		snd_ctl_elem_value_set_enumerated (
			_snd_ctl_elem_value, ii, enum_idx_n );
	}
	value_was_set();
}
	...
	unsigned int mask_n )
{
	...
	if ( smce != 0 ) {
			...
			( ( mask_n & change_mask ) != 0 ) )
		{
			...
			// Unusual mask
			::std::cerr << "Mixer_HCTL_Elem::alsa_callback_hctl_elem: ";
