...
ALSA_Config_Watcher::ALSA_Config_Watcher (
	::QObject * parent_n ) :
QObject ( parent_n )
{
	...
	connect ( &_fwatch, SIGNAL ( fileChanged ( const QString & ) ),
		this, SLOT ( change_in_file ( const QString & ) ) );

	connect ( &_fwatch, SIGNAL ( directoryChanged ( const QString & ) ),
		this, SLOT ( change_in_dir ( const QString & ) ) );
}


void
ALSA_Config_Watcher::change_in_file (
	const QString & fl_n )
{
...
void
ALSA_Config_Watcher::change_in_dir (
	const QString & fl_n )
{
	if ( fl_n == _cfg_system_dir ) {
		if ( !_fwatch.files().contains ( _cfg_system_file ) ) {
			if ( QFile::exists ( _cfg_system_file ) ) {
				//::std::cout << "Change in dir " << _cfg_system_file.toLocal8Bit().data() << "\n";
				_fwatch.addPath ( _cfg_system_file );
				emit sig_change();
			}
		}
