...
void
Mixer_Simple_Filter_Name::set_blacklist (
	bool flag_n )
{
	_blacklist = flag_n;
}
...
void
Mixer_Simple_Filter_Name::append_name (
...
void
Mixer_Simple_Filter_Name::append_names (
		...
		while ( aii < accept_n.size() ) {
			...
			if ( elem != 0 ) {
				...
				for ( int dii=0; dii < _names.size(); ++dii ) {
					...
					if ( ::QString::compare ( cmp_str,
						elem->elem_name(), Qt::CaseInsensitive ) == 0 )
					{
						match = true;
					} else if ( ::QString::compare ( cmp_str,
						elem->display_name(), Qt::CaseInsensitive ) == 0 )
					{
						match = true;
					}
					if ( match ) {
