...
Mixer_HCTL::create_mixer_elems ( )
{
	...
	while ( hctl_elem != 0 )	{
		...
		snd_hctl_elem_set_callback ( hctl_elem, &Mixer_HCTL_Elem::alsa_callback_hctl_elem );
...
void
Mixer_HCTL::set_socked_notifiers_enabled (
	bool flag_n )
{
	for ( int ii=0; ii < _socket_notifiers.size(); ++ii ) {
