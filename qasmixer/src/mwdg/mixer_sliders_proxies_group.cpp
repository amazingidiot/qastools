...
Mixer_Sliders_Proxies_Group::Mixer_Sliders_Proxies_Group (
	QObject * parent_n ) :
...
void
Mixer_Sliders_Proxies_Group::set_snd_dir (
	unsigned char dir_n )
{
	_snd_dir = dir_n;
}
...
void
Mixer_Sliders_Proxies_Group::set_is_joined (
	bool flag_n )
{
	_is_joined = flag_n;
}
...
void
Mixer_Sliders_Proxies_Group::set_is_visible (
	bool flag_n )
{
	_is_visible = flag_n;
}
...
void
Mixer_Sliders_Proxies_Group::set_notify_value_change (
	bool flag_n )
{
...
void
Mixer_Sliders_Proxies_Group::set_separation_request (
	bool flag_n )
{
...
Mixer_Sliders_Proxies_Group::should_be_separated ( ) const
{
	...
	if ( sme != 0 ) {
		res =
			( ( sme->num_volume_channels ( snd_dir() ) > 1 ) && !sme->volumes_equal ( snd_dir() ) ) ||
...
Mixer_Sliders_Proxies_Group::update_mixer_values ( )
{
	for ( unsigned int ii=0; ii < num_columns(); ++ii ) {
		static_cast < Mixer_Sliders_Proxies_Column * > (
			column ( ii ) )->update_mixer_values();
	}
...
bool
Mixer_Sliders_Proxies_Group::event (
	QEvent * event_n )
{
