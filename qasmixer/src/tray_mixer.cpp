...
_balloon ( 0 )
{
	...
	// Tray icon
	{
		...
		connect ( _tray_icon, SIGNAL ( sig_quit() ),
			this, SIGNAL ( sig_quit() ) );

		connect ( _tray_icon, SIGNAL ( sig_hover() ),
			this, SLOT ( raise_balloon() ) );

		connect ( _tray_icon, SIGNAL ( sig_activated() ),
			this, SIGNAL ( sig_toggle_mixer() ) );

		connect ( _tray_icon, SIGNAL ( sig_wheel_delta ( int ) ),
			this, SLOT ( mouse_wheel_delta ( int ) ) );

		connect ( _tray_icon, SIGNAL ( sig_middle_click() ),
...
Tray_Mixer::init_icons ( )
{
	...
	_icons_volume[1] = QIcon::fromTheme ( "audio-volume-muted", _icons_volume[2] );
}


void
Tray_Mixer::set_mdev_setup (
