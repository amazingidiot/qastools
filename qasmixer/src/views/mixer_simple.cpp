...
#include <mview/mv_mixer_simple_setup.hpp>
...
_qsnd_mixer ( 0 )
{
	...
	// Mixer switches
	{
		...
		connect ( _btn_stream[0], SIGNAL ( toggled ( bool ) ),
			this, SLOT ( show_playback ( bool ) ), Qt::QueuedConnection );

		connect ( _btn_stream[1], SIGNAL ( toggled ( bool ) ),
			this, SLOT ( show_capture ( bool ) ), Qt::QueuedConnection );
	}
...
MV_Mixer_Simple::setup_view ( )
{
	...
	if ( lay_stack_idx == 1 ) {
		...
		bool vis_stream_select (
			( _qsnd_mixer->num_elems_stream ( 0 ) > 0 ) &&
			( _qsnd_mixer->num_elems_stream ( 1 ) > 0 ) );
